...
import { handleJamendoRequestSafely } from './handler.js'
 ...
 * Vercel Function runs — same validation, same clamping, same sanitization,
 * same redaction (agents/16_JAMENDO_SERVERLESS_SECURITY.md → "Local
 * Development").
 *
...
export function createJamendoMiddleware(options: MiddlewareOptions) {
  return function jamendoMiddleware(
    request: IncomingMessage,
    response: ServerResponse,
    next: () => void,
  ): void {
    const path = (request.url ?? '').split('?')[0]
    if (path !== JAMENDO_ROUTE) {
      next()
      return
    }
    ...
    void (async () => {
      ...
      const webResponse = await handleJamendoRequestSafely(webRequest, { env: options.env })
