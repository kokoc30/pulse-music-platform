...
from how that failure was *shared*.
...
The lifecycle now guarantees the opposite invariant — *a failed, timed-out,
detached, superseded or never-ready construction must never prevent a later real
start from building a functional player*:
...
- **No player exists** — the press *is* the recovery. It resets the start guard
...
A construction abandoned on its bound is a failure to *start*, not a failure of
...
and the sheet shows one quiet line — *"Tap play to continue this YouTube
track."* — which it did not show for this reason before, because before, the
...
The engine now republishes the position as `(0, 0)` the moment a *different*
...
The readout gained the facts whose absence made this unplaceable on a device
...
`iframe autoplay` now reads the real frame **after** the player is built
...
player is constructed *empty* has been reversed to require the video-seeded
...
pnpm test:run      pass — 2081 tests, 108 files
pnpm build         pass
pnpm test:e2e      pass — 529 passed, 29 skipped
pnpm verify:bundle pass — 0 matches across 12 files
```

## Physical-device status

Not yet verified on hardware. The deterministic gates cover the mechanism —
the stall, the bound, the supersession, the late arrival and the Play-button
recovery — but the original defect only reproduced on a physical phone, and only
a physical phone can confirm the fix.

What to check, with `?debugPlayback=1`:

1. Audio A → YouTube B: B should start on its own.
2. If it does not, the readout must now say **which** of the two it was —
   `player creation: timed-out` (this app's problem) or a play command in
   `commands` with a refusal in `states` (the browser's).
3. Either way, **Play must start B on the first press.** A black player whose
   Play button does nothing is never acceptable, and that specific outcome is
